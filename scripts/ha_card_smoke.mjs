...
const DIST_FILE = "dist/ha-dhe-connect-card.js";
...
async function validateBundle() {
  ...
  if (!bundle.includes("dhe-connect-card")) {
...
async function deployBundle() {
  ...
  await copyWithRetry(DIST_FILE, path.join(deployDir, "ha-dhe-connect-card.js"));
  await copyWithRetry(`${DIST_FILE}.map`, path.join(deployDir, "ha-dhe-connect-card.js.map"));
...
async function validateHaStates(accessToken) {
  ...
  const dheStates = states.filter((state) => {
    const entityId = String(state.entity_id || "").toLowerCase();
    const friendlyName = String(state.attributes?.friendly_name || "").toLowerCase();
    return entityId.includes("dhe") || friendlyName.includes("dhe") || friendlyName.includes("stiebel");
  });
  const hasClimate = dheStates.some((state) => String(state.entity_id).startsWith("climate."));
  const hasControl = dheStates.some((state) =>
    /^(switch|number|button|select|media_player)\./.test(String(state.entity_id)),
  );
  if (!dheStates.length) {
    throw new Error("no DHE-like Home Assistant states found");
  }
  if (!hasClimate) {
    throw new Error("no DHE climate state found");
  }
  if (!hasControl) {
    throw new Error("no DHE control entities found");
  }
  pass(`HA API exposes ${dheStates.length} DHE-like states`);
}
...
async function canWriteDirectory(directory) {
  const probe = path.join(directory, `ha-dhe-connect-card-smoke-${process.pid}.tmp`);
  try {
...
async function upsertLovelaceResource(accessToken) {
  ...
  for (const item of existing) {
    if (String(item.url || "").includes("/ha-dhe-connect-card/ha-dhe-connect-card.js")) {
...
function resourceUrlFromDeployDir() {
  ...
  const relativeFile = path
    .join(relativeDir, "ha-dhe-connect-card.js")
    .split(path.sep)
    .filter(Boolean)
    .join("/");
...
try {
  ...
  try {
    pass(token.source === "long-lived-token" ? "HA auth token accepted" : "HA auth login");
